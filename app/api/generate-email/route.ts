...
async function handler(req: NextRequest) {
  try {
    ...
    const systemPrompt = `You are an AI assistant helping a visitor draft an email to  Vikneshwaran. 
    The email should be written from the perspective of the visitor/user, addressed to Vikneshwaran.
    Use the user's prompt to determine the intent (e.g., job offer, collaboration, question).
    
    Requirements:
    - Address the email to "Viknesh" or "Mr.Vicky".
    ...
    const response = await fetch(
      "https://openrouter.ai/api/v1/chat/completions",
      {
        ...
        headers: {
          ...
          "X-Title": "Vikneshwaran's Portfolio",
